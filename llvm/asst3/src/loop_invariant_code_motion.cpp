    ...
    public:
        ...
        virtual bool runOnLoop(Loop * L, LPPassManager & LPM)
        {
                ...
                for(BasicBlock* bb : L->blocks()){
                    ...
                    if(loopinfo->getLoopFor(bb) == L){
                        ...
                        for(Instruction& inst : *bb){
                            // 首先判断是否在 不变的 集合中，且判断这个 Inst 是不是在 循环中不变的。
                            // 如果是 就加入到 markedAsInvariant 集合中
                            if(FOUND(markedAsInvariant, &inst) && isInvariant(&inst, L)){
                                // 如果是 循环不变， 加入 markedAsInvariant 中。
                                hasChanged = true;
            ...
            for(Instruction* inst : markedAsInvariant)
            {
                ...
                */
                // 由于SSA的特殊性 所以我们只用 严重 1. s所在的基本块是循环所有出口结点(有后继结点在循环外的结点)的支配结点
